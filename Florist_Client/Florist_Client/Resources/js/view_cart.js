...
function check_back(month, month_current) {
    if (month_current <= month) {
        $('.prev-month').addClass('history_2');
    } else {
        $('.prev-month').removeClass('history_2');
    }
...
function renderCalendar(startDay, totalDays, currentDate, month) {
    ...
    for (; i <= totalDays; i++) {
        ...
        $day.click(function (e) {
            ...
            $('#date_buy').val(day_ship)
        });
        if (month_crrent == month && i < currentDate) {
            $day.addClass('history_2');
        }
...
$('.time_buy_now').each(function (ix, e) {
    $(e).click(function (element) {
        removeClassTime();
        $(this).addClass("today")
        $('#time_buy').val($(this).html())
    })
})
